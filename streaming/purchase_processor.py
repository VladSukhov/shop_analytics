...
def get_kafka_stream(spark: SparkSession) -> DataFrame:
    ...
    следующими колонками:
      purchase_id, customer_id, product_id, quantity, amount, purchase_time
    """
    ...
    # 2) Определяем схему JSON для поля value
    purchase_schema = StructType([
        ...
        StructField("purchase_time", TimestampType(), nullable=False),
    ])
    ...
    purchases_df = (
        ...
        .withColumn(
            "purchase_time", 
            expr("CASE WHEN purchase_time IS NULL THEN kafka_timestamp ELSE purchase_time END")
        )
...
def build_windowed_aggregates(enriched_df: DataFrame) -> DataFrame:
    ...
    agg_df = (
        ...
        .groupBy(
            window(col("purchase_time"), "1 hour", "10 minutes").alias("time_window"),
            col("product_category")
        )
        .agg(
            {"*": "count", "amount": "sum", "quantity": "sum"}
        )
